...
# ---------------------------------------------------------------------------------------------------------------------
# -GLOBAL OBJECTS------------------------------------------------------------------------------------------------------
_index = 'openalex'  # any from the list ['econbiz', 'sowiport', 'openalex', 'crossref', 'ssoar', 'gesis_bib', 'arxiv', 'research_data']
_scroll_size = 100  # how many input docs to retrieve at a time from the index
...
def map_authors(index, authlist):
    authors = []
    if index == 'econbiz' or index == 'sowiport' or index == 'ssoar' or index == 'gesis_bib' or index == 'research_data':
...
def extract_ints(text):
    ints = [int(s) for s in re.findall(r'\b\d+\b', text)]
    return ints
...
def get_collections_refstrings():
    ...
    while returned > 0:
        for doc in page['hits']['hits']:
            ...
            try:
                print(doc['_id'], ' count: ', i)
                i += 1
                ref_obj = collectionObj2modelObj(_index, doc)
                ref_str = json_to_apa(ref_obj)
                # print(ref_obj)
                # print(ref_str)
